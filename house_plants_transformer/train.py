"""
Training Script for House Plant Vision Transformer
...
from model import create_houseplant_vit_small, create_houseplant_vit_tiny, create_houseplant_vit_base, create_houseplant_vit_pretrained
from dataset import create_dataloaders


class Trainer:
    """
    Trainer class for House Plant Vision Transformer.
    """
    ...
    def __init__(
        ...
        scheduler,
        device,
        save_dir='checkpoints_houseplants',
        use_amp=True,
...
def main():
    ...
    print("House Plant Vision Transformer Training")
    print("="*60)
    print("Configuration:")
    ...
    if config['use_pretrained'] and config['model_size'] in ['tiny', 'small', 'base']:
        ...
        try:
            model = create_houseplant_vit_pretrained(
    ...
    if not config['use_pretrained']:
        ...
        if config['model_size'] == 'tiny':
            model = create_houseplant_vit_tiny(
                num_classes=num_classes,
                img_size=config['img_size'],
                use_checkpoint=config['use_checkpoint']
            )
        elif config['model_size'] == 'small':
            model = create_houseplant_vit_small(
                num_classes=num_classes,
                img_size=config['img_size'],
                use_checkpoint=config['use_checkpoint']
            )
        else:  # base
            model = create_houseplant_vit_base(
                num_classes=num_classes,
                img_size=config['img_size'],
                use_checkpoint=True
            )
    ...
    if config['learning_rate'] is None:
        if pretrained_loaded:
            config['learning_rate'] = 3e-4  # Fine-tuning LR
