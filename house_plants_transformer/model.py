"""
Vision Transformer for House Plant Classification

A Vision Transformer implementation adapted for house plant species identification.
Based on the same architecture as the broad plant classifier but optimized for
indoor plant species recognition.
...
- Layer normalization
- Designed for 47 house plant species
"""
...
class HousePlantViT(nn.Module):
    """
    Vision Transformer for house plant species classification.

    Optimized for:
    - 224x224 RGB images (standard resolution)
    - 8GB VRAM constraint
    - 47 house plant species classification
    """
    def __init__(
        self,
        img_size=224,
        ...
        Args:
            ...
            num_classes: Number of output classes (default: 47 for house plants)
...
def create_houseplant_vit_small(num_classes=47, img_size=224, use_checkpoint=False):
    """
    Create a small Vision Transformer optimized for house plant classification.
...
def create_houseplant_vit_tiny(num_classes=47, img_size=224, use_checkpoint=False):
    """
...
def create_houseplant_vit_base(num_classes=47, img_size=224, use_checkpoint=True):
    """
    Create a base Vision Transformer for house plants (larger model, requires gradient checkpointing).
...
def create_houseplant_vit_pretrained(model_name='vit_base_patch16_224', num_classes=47, pretrained=True):
    """
    Create a pretrained Vision Transformer for house plant classification using timm library.
    ...
    Args:
        ...
        num_classes: Number of output classes (47 for house plants)
...
if __name__ == "__main__":
    # Test the model
    print("Testing House Plant Vision Transformer...")

    # Create model
    model = create_houseplant_vit_small(num_classes=47, img_size=224)

    # Count parameters
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

    print(f"\nModel: HousePlantViT-Small")
    ...
    print("\nHouse plant species supported: 47")
